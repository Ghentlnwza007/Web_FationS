...
// =============================================
export default function OrderHistory({ userId, refreshOrderCount }) {
  ...
  const getStepStatus = (orderStatus, step) => {
    ...
    // 'paid' status means payment completed - treat as past 'processing'
  ...
  const confirmAction = async () => {
    ...
    try {
      // Always update local storage regardless of source
      const localOrders = JSON.parse(localStorage.getItem('maison_orders') || '[]');
      ...
      // If it's a Firestore order, also update Firestore
